...
public class CustomerDAO extends JdbcDaoSupport{
	...
	public ArrayList<Customer> getAllCustomerDetails() {
	    ...
	    List<Customer> c1 =  createJdbcTemplate().query(
	        sql,
	        (rs, rowNum) -> {
	            Customer c = new Customer();
	            c.setCustomerId(rs.getInt("customer_id"));
	...
	public ResponseEntity<Object> deleteCustomer(int cid) {
		 int rowsAffected = createJdbcTemplate().update("DELETE FROM customers WHERE customer_ID = ?", cid);
	        if (rowsAffected > 0)
